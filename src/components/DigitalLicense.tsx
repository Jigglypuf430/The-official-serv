...
1) Replace the <DigitalLicence /> component below with YOUR React component (paste over the placeholder).
2) Keep the default export (PreviewApp) as-is — it renders your component inside a clean preview frame.
3) If your component relies on images/assets, reference remote URLs or inline data URIs.
4) Tailwind is available. You can also import any npm package.
*/

// ====== Small pure helpers + dev tests ======
export const clamp = (value: number, min: number, max: number) => Math.min(Math.max(value, min), max);
export function mapTiltToTarget(gamma: number, beta: number, sensitivity = 1.6) {
...
(function devTests() {
  try {
    ...
    // mapTiltToTarget tests
    const mid = mapTiltToTarget(0, 0, 1.6);
    console.assert(Math.abs(mid.x - 50) < 1e-6 && Math.abs(mid.y - 50) < 1e-6, "map tilt center");
    ...
    console.assert(leftDown.x >= 10 && leftDown.x <= 20, "map tilt left clamped >=10");
    console.assert(leftDown.y >= 10 && leftDown.y <= 20, "map tilt down clamped >=10");
...
// ⬇️ Replace everything inside here with YOUR component code
function DigitalLicence() {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const canvasRef = useRef<HTMLCanvasElement | null>(null); const specularRef = useRef<HTMLDivElement | null>(null);
  ...
  // Motion state (we only drive shader parallax; the card itself stays put)
  const currentPos = useRef({ x: 50, y: 50 });
  const targetPos = useRef({ x: 50, y: 50 });
  const isAnimating = useRef(false);
  const sensitivity = 1.6;

  // ---- Device / pointer input → targetPos ----
  const handleOrientation = (event: DeviceOrientationEvent) => {
    if (!permissionGranted) return;
    const gamma = event.gamma ?? 0; // x tilt
    const beta = event.beta ?? 0; // y tilt
  ...
  // ---- Smooth lerp towards targetPos (we render on-demand while moving) ----
  const animateLerp = () => {
    const speed = 0.12;
    ...
    // Move specular highlight with input
  ...
  // ---- THREE.js metaball shader setup ----
  useEffect(() => {
    ...
    // Time-free, input-driven liquid pools (metaballs + noise warp)
    const fragmentShader = `
      precision highp float;
      varying vec2 vUv;
      uniform vec2 u_tilt;  // 0..1 (x,y)
    ...
    // Avoid optional chaining with new
    ...
    return () => {
      ...
      // TS-safe cleanup using a ref
      const r = roRef.current;
      const el = containerRef.current;
      if (r && el) {
        try { r.unobserve(el); } catch (_) {}
      }
      if (r) {
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      <style>{`
        ...
        .specular-overlay{
          ...
          -webkit-mask-image:url('https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/68cfe7e4208081a124aa9e75/49e6df419_warratah.png');
          mask-image:url('https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/68cfe7e4208081a124aa9e75/49e6df419_warratah.png');
          -webkit-mask-size:contain; mask-size:contain; -webkit-mask-position:center; mask-position:center; -webkit-mask-repeat:no-repeat; mask-repeat:no-repeat;
            ...
            <div className="relative z-30 px-4 pt-2 pb-6">
              <section className="relative flex justify-between items-start h-24">
                <div
                  ...
                  style={{ backgroundImage: "url('https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/68cfe7e4208081a124aa9e75/95e347781_nsw-logo.png')" }}
                ></div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <section className="p-5 rounded-2xl bg-white/5 shadow-xl space-y-3">
            ...
            <ul className="list-disc pl-5 text-slate-300 text-sm space-y-2">
              <li>
                Keep <code>export default PreviewApp</code> — replace only the inner component (e.g.,
